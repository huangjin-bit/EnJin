...
def render_program(program: Program, config: RenderConfig) -> None:
    ...
    # 代码分离：separate_generated=True 时，生成代码输出到 generated/ 子目录
    separate = _is_separate_generated(app_config, config.target_lang)
    base_dir = config.output_dir / config.target_lang
    output_dir = base_dir / "generated" if separate else base_dir
    ...
    # 代码分离：生成 custom/ 目录骨架
    if separate:
        _create_custom_skeleton(base_dir, config.target_lang, config.app_name)
...
def _is_separate_generated(app_config: dict, target_lang: str) -> bool:
    """Check if code separation is enabled in layout config."""
    from enjinc.layout_config import get_java_layout, get_python_layout
    if target_lang == "java_springboot":
        return get_java_layout(app_config).separate_generated
    elif target_lang == "python_fastapi":
        return get_python_layout(app_config).separate_generated
    return False


def _create_custom_skeleton(base_dir: Path, target_lang: str, app_name: str) -> None:
    """Create custom/ directory with extension stubs for user-maintained code."""
    custom_dir = base_dir / "custom"
    custom_dir.mkdir(parents=True, exist_ok=True)

    if target_lang == "java_springboot":
        pkg = app_name.replace("-", "_")
        pkg_path = custom_dir / "src" / "main" / "java" / pkg
        for sub in ("application/service/impl", "infrastructure/mapper", "interfaces/controller"):
            (pkg_path / sub).mkdir(parents=True, exist_ok=True)
    elif target_lang == "python_fastapi":
        pkg = "app"
        for sub in ("services", "repositories", "api/v1", "core"):
            (custom_dir / pkg / sub).mkdir(parents=True, exist_ok=True)

    # README explaining the separation
    (custom_dir / "README.md").write_text(
        "# Custom Code Directory\n\n"
        "This directory contains your hand-written code that will not be overwritten by EnJin.\n\n"
        "## Structure\n\n"
        "- `generated/` — Auto-generated by `enjinc build`. **Do not edit** — changes will be lost.\n"
        "- `custom/` — Your custom code. Safe to edit, EnJin will never touch this.\n\n"
        "## How to extend\n\n"
        "1. Import from `generated/` package and extend/override in your custom code.\n"
        "2. Register your custom implementations in the dependency injection container.\n",
        encoding="utf-8",
    )
